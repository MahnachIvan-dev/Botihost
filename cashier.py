...
async def cmd_set_ref(message: types.Message, state: FSMContext):
    ...
    await message.answer("📸 Отправь скриншот своего профиля/подарка, который будет эталоном для ИИ:")
...
async def process_set_ref(message: types.Message, state: FSMContext):
    ...
    await message.answer("✅ <b>Эталонный скриншот сохранен!</b> ИИ будет использовать его при сомнениях.")
    await state.clear()
