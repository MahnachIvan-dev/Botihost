...
async def analyze_two_images(img1,img2,plan_id,tz_offset,tz_name):
    ...
    profile_ref=await file_b64(ref_file("profile")); payment_ref=await file_b64(ref_file("payment"))
    prompt=f"""
...
"""
    ...
    if profile_ref:
        content += [{"type":"text","text":"ADMIN REFERENCE PROFILE (только для сравнения профиля):"},{"type":"image_url","image_url":{"url":f"data:image/jpeg;base64,{profile_ref}"}}]
    if payment_ref:
        content += [{"type":"text","text":"ADMIN REFERENCE PAYMENT (только как визуальный пример):"},{"type":"image_url","image_url":{"url":f"data:image/jpeg;base64,{payment_ref}"}}]
    try:
...
async def profile_photo(message:types.Message,state:FSMContext):
    data=await state.get_data(); plan=data["plan_id"]; msg=await message.answer("⏳ <i>Сверяю чек, профиль, эталон профиля и время…</i>",parse_mode="HTML")
    try:
        ...
        grant_id=uuid4().hex; ok=await finish_result(message.from_user.id,plan,"APPROVED",grant_id)
        if not ok:
