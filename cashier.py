...
PAID_SERVICE_PLANS={"video":{"name":"🎬 Видео","stars":15,"uses":5},"animation":{"name":"✨ Анимации","stars":15,"uses":5}}
...
async def analyze_two_images(img1,img2,plan_id,tz_offset,tz_name):
    ...
    if not plan: return {"is_valid":False,"reason":"Неизвестный тип оплаты."}
...
async def finish_service_result(uid,service_key,result,grant_id):
    sig=sign("service_result",uid,service_key,result,grant_id)
    await send_channel(f"/service_payment_result {uid} {service_key} {result} {grant_id} {sig}")
...
async def sync_channel_post(message:types.Message):
    ...
    if text.startswith("/payment_request"):
        ...
        kb=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"💳 Оплатить {PLANS[plan]['name']} · {PLANS[plan]['stars']}⭐",callback_data=f"plan:{plan}")]])
        try:
            await bot.send_message(uid,f"💳 <b>Оплата заказа</b>\n\nТариф: <b>{PLANS[plan]['name']}</b> · <b>{PLANS[plan]['stars']}⭐</b>\n\nНажми кнопку, чтобы пройти проверку оплаты через кассир.",reply_markup=kb,parse_mode="HTML")
        except Exception as e: logging.error("payment request user notify: %s",e)
        return
    ...
    if len(parts)<6:
        return
    try: uid=int(parts[1]); service_key=parts[2]; uses=int(parts[3]); stars=int(parts[4]); request_id=parts[5].splitlines()[0].strip()
    except Exception:
        return
    ...
    if SYNC_SECRET and not hmac.compare_digest(expected,signature):
        logging.warning("Неверная подпись service_payment_request для %s",uid); return
    kb=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"💳 Оплатить {stars}⭐",callback_data=f"service_start:{request_id}:{service_key}")]])
    try:
        await bot.send_message(uid,f"💎 <b>Платная услуга</b>\n\n{service['name']}\n🎟 Пакет: <b>{uses} использований</b>\n💰 Стоимость: <b>{stars}⭐</b>\n\nНажми кнопку, чтобы пройти проверку оплаты через кассир.",reply_markup=kb,parse_mode="HTML")
    except Exception as e:
        logging.error("service request user notify: %s",e)
...
async def service_start(call:types.CallbackQuery,state:FSMContext):
    ...
    allowed,reason=guard_status(call.from_user.id)
    if not allowed:
        return await call.answer("Повторная покупка пока недоступна",show_alert=True)
    ...
    await call.message.answer(f"💎 <b>{service['name']}</b>\n\nПакет: <b>{service['uses']} использований</b> · <b>{service['stars']}⭐</b>\n\n🌍 <b>Шаг 1 из 3</b>\nВыбери часовой пояс для проверки времени на чеке.",reply_markup=InlineKeyboardMarkup(inline_keyboard=kb),parse_mode="HTML")
    ...
    try:
        ...
        async with payment_guard_lock:
            ...
            if plan in PAID_SERVICE_PLANS:
                ok=await finish_service_result(message.from_user.id,plan,"APPROVED",grant_id)
            else:
                ok=await finish_result(message.from_user.id,plan,"APPROVED",grant_id)
            if not ok:
                await msg.edit_text("💳 Оплата проверена, но не удалось передать результат в BotHost. Администратор получит уведомление.",parse_mode="HTML"); return
            await commit_approved_payment(message.from_user.id,payment_b64,plan,grant_id)
        cfg=PLANS.get(plan) or PAID_SERVICE_PLANS.get(plan)
        label=cfg['name']
        extra=f"Пакет: <b>{cfg['uses']} использований</b>." if plan in PAID_SERVICE_PLANS else f"Тариф: <b>{label}</b>"
        await msg.edit_text(f"🎉 <b>Оплата подтверждена!</b>\n\n{extra}\n\nРезультат передан BotHost.\n\n⏱ Повторная покупка будет доступна через <b>1 час</b>.",parse_mode="HTML"); await state.clear()
