...
import os, sys, re, subprocess, time, venv
from pathlib import Path

ENTRY_POINT = "{{ENTRY_POINT}}"
BASE_DIR = Path.cwd()
VENV_DIR = BASE_DIR / ".venv"
BLOCKED_HOST_KEYS = {"BOT_TOKEN", "CASHIER_TOKEN", "GROQ_API_KEY", "SYNC_CHANNEL_ID", "SYNC_SECRET", "OWNER_ID", "OWNER_USERNAME", "VERIFIER_BOT", "DATA_DIR", "DATABASE_URL", "RAILWAY_TOKEN", "RAILWAY_SERVICE_ID"}
IMPORT_TO_PACKAGE = {"PIL":"Pillow", "cv2":"opencv-python", "bs4":"beautifulsoup4", "dotenv":"python-dotenv", "yaml":"PyYAML", "Crypto":"pycryptodome", "dateutil":"python-dateutil", "jwt":"PyJWT", "multipart":"python-multipart", "fitz":"PyMuPDF"}
...
def requirements_files():
    ...
    for p in BASE_DIR.rglob("*.txt"):
        ...
        n=p.name.lower()
        if n == "requirements.txt" or ("requirements" in n and n.endswith(".txt")): out.append(p)
    return sorted(set(out))

def make_venv():
    py=VENV_DIR/"bin"/"python"
    if not py.exists():
        print("[ BotHost ] Создаю отдельное окружение Python...", flush=True)
...
def install_requirements(py):
    ...
    if not files:
        print("[ BotHost ] requirements*.txt не найден — зависимости будут определяться по ошибкам импорта.", flush=True)
        return True
    import hashlib
    digest=hashlib.sha256()
    for req in files:
        digest.update(str(req.relative_to(BASE_DIR)).encode())
        digest.update(req.read_bytes())
    marker=BASE_DIR/".requirements.hash"
    current=digest.hexdigest()
    if marker.exists() and marker.read_text().strip()==current:
        print("[ BotHost ] ✅ Зависимости уже установлены.", flush=True)
        return True
    for req in files:
        print(f"[ BotHost ] 📦 Устанавливаю зависимости: {req.relative_to(BASE_DIR)}", flush=True)
        r=subprocess.run([str(py),"-m","pip","install","-r",str(req),"--disable-pip-version-check","--no-cache-dir"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
        print(r.stdout[-12000:], flush=True)
        if r.returncode != 0: return False
    marker.write_text(current, encoding="utf-8")
    return True

def install_missing(py, output):
    m=re.search(r"No module named (.+)", output or "")
    if not m: return False
    module=m.group(1).split()[0].replace(chr(39),"").replace(chr(34),"").split(".")[0]; package=IMPORT_TO_PACKAGE.get(module,module)
    print(f"[ BotHost ] 🔎 Не хватает {module}; устанавливаю {package}", flush=True)
    r=subprocess.run([str(py),"-m","pip","install",package,"--disable-pip-version-check","--no-cache-dir"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
    print(r.stdout[-12000:], flush=True)
    return r.returncode == 0

def build_env():
    env={k:v for k,v in os.environ.items() if k not in BLOCKED_HOST_KEYS and not k.startswith("RAILWAY_")}
    env.update(load_dotenv(BASE_DIR/".env")); env["PYTHONUNBUFFERED"]="1"
    return env

def run():
    print(f"[ BotHost ] Подготовка проекта... Точка входа: {ENTRY_POINT}", flush=True)
    env=build_env()
    try: py=make_venv()
    except Exception as e:
        print(f"[ BotHost ] ⚠️ Не удалось создать venv: {e}. Использую системный Python.", flush=True); py=Path(sys.executable)
    if not install_requirements(py):
        print("[ BotHost ] ❌ Ошибка установки зависимостей. Проверьте requirements*.txt.", flush=True); sys.exit(2)
    for attempt in range(3):
        print(f"[ BotHost ] 🚀 Запуск (попытка {attempt+1}/3): {ENTRY_POINT}", flush=True)
        proc=subprocess.Popen([str(py),"-u",ENTRY_POINT],env=env,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,bufsize=1)
        output=[]
        while True:
            line=proc.stdout.readline()
            if line: print(line,end="",flush=True); output.append(line)
            elif proc.poll() is not None: break
            else: time.sleep(0.05)
        ret=proc.returncode
        if ret == 0: sys.exit(0)
        if attempt < 2 and install_missing(py,"".join(output)):
            print("[ BotHost ] 🔄 Зависимость установлена, повторяю запуск...",flush=True); continue
        sys.exit(ret or 1)
...
async def start_user_bot(bot_id):
    try:
        ...
        # Обновляем wrapper и для старых ботов, чтобы они получили новый менеджер зависимостей.
        entry_point = b[7] or "user_bot.py"
        ...
        blocked = {
            "BOT_TOKEN", "CASHIER_TOKEN", "GROQ_API_KEY", "SYNC_CHANNEL_ID",
            "SYNC_SECRET", "OWNER_ID", "OWNER_USERNAME", "VERIFIER_BOT",
            "DATA_DIR", "DATABASE_URL", "RAILWAY_TOKEN", "RAILWAY_SERVICE_ID",
        }
        env = {k: v for k, v in os.environ.items()
               if k not in blocked and not k.startswith("RAILWAY_")}
...
async def analyze_with_groq(logs):
    ...
    try:
        response = await groq_client.chat.completions.create(
            ...
            model="qwen/qwen3.8-27b", temperature=0.2, max_tokens=400
        )
